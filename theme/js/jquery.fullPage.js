	...
	$.fn.fullpage = function(options) {
		...
		$('.section').each(function(index){
			...
			if (numSlides > 0) {
				...
				slides.each(function(index) {
					...
					if(!index && that.find('.slider.active').length == 0){
						$(this).addClass('active');
					}
			...
			var activeSlide = $('.section.active').find('.slider.active');
		...
		*/		
				...
				if (!isMoving && !slideMoving) { //if theres any #
					...
					//vertical scrolling (only when autoScrolling is enabled)
						...
						if(activeSection.find('.slides').length){
							scrollable= activeSection.find('.slider.active').find('.scrollable');
						}else{
		...
		function moveSlide(direction){
		    ...
		    var currentSlide = slides.find('.slider.active');
		    var destiny = null;

		    if(direction === 'prev'){
		        destiny = currentSlide.prev('.slider');
		    }else{
		        destiny = currentSlide.next('.slider');
		    }
		...
		function scrollPage(element, callback, isMovementUp){
			...
			var activeSlide = element.find('.slider.active');
			var activeSection = $('.section.active');
		...
		$('.section').on('click', '.toSlide', function(e) {
			...
			destiny = slides.find('.slider').eq( ($(this).data('index') -1) );
		...
		*/
		function landscapeScroll(slides, destiny){
			...
			if(options.onSlideLeave){
				var prevSlideIndex = section.find('.slider.active').index();
		...
		function doneResizing() {
			...
			$('.section').each(function(){
				...
				if (slides.length) {
					landscapeScroll(slides, slides.find('.slider.active'));
				}
			});
		...
		*/
		function scrollSlider(section, slide){
			if(typeof slide != 'undefined'){
				...
				if(!destiny.length){
					destiny = slides.find('.slider').eq(slide);
				}
		...
		*/
		$(document).on('click', '.fullPage-slidesNav a', function(e){
			e.preventDefault();
			var slides = $(this).closest('.section').find('.slides');		
			var destiny = slides.find('.slider').eq($(this).closest('li').index());
